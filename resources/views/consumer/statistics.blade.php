<x-app-layout>
    ...
    <div class="row pt-4 mt-2">
        <form action="{{route('consumer-search')}}" method="post">
            @csrf
            <livewire:consumer.search-input />
                        ...
                        @if($markers)
                            <div class="flex justify-between">
                                <div>Количество посаженных деревьев: {{$markers->total()}}</div>
                                    <div>
                                        <livewire:consumer.export-marker :forExp="$forExp"/>
                                    </div>
                            </div>
                                ...
                                <tbody>
                                    @foreach($markers as $item)
                                        <tr class="border-b dark:border-neutral-500">
                                            ...
                                            <td class="whitespace-nowrap px-6 py-4">
                                                <a href="{{route('consumer-marker-show', ["id"=>$item->id])}}" class="mr-3">
