<x-app-layout>
    ...
    <div class="container mx-auto py-5">
        <form id="area-form" action="{{route('private-policy.update', $policy->id)}}" method="post">
            @csrf
            @method('PUT')
            <div class="my-2">
                <label>На казахском</label>
                <textarea name="text_kk">{{$policy->text_kk}}</textarea>
            </div>
            <div class="my-2">
                <label>На русском</label>
                <textarea name="text_ru">{{$policy->text_ru}}</textarea>
            </div>
            <button
                id="submit-map"
                type="submit"
